 ...
 */
public class IndexibleMap<V extends Identifable> implements Iterable<V> {
	...
	public V put(V v, int pos) {
		...
		positions.put(pos, v.getId());
		if (existing != null) {
			positions.put(values.size(), existing.getId());
			values.put(existing.getId(), existing);
		} 
		return existing;
	}
